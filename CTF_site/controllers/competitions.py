...
def create_competition(db, current_user, comp_body):
    ...
    new_team = Team(name=comp_body.owner_team_name,
                    competition_id=new_competition.id)  # competition_id will be set later
    db.add(new_team)
    db.commit()
...
def encode_image_from_binary(binary_data: bytes) -> str:
    ...
    for team in teams:
        ...
        if competition:
            ...
            for task, points in tasks_with_details:
                ...
                if task.image:
                    try:
                        image_data = encode_image_from_binary(task.image)  # Encode binary data
